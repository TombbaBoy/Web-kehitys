...
// Euclidean distance function (longitude/latitude coordinates)
function getDistance(lat1, lon1, lat2, lon2) {
    return Math.sqrt(Math.pow(lat1 - lat2, 2) + Math.pow(lon1 - lon2, 2));
}

// Function to populate table with restaurants
...
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
        ...
        const sortedRestaurants = restaurants.slice().sort((a, b) => {
            ...
            const distA = getDistance(userLat, userLon, latA, lonA);
            const distB = getDistance(userLat, userLon, latB, lonB);
        ...
        td.textContent = 'Could not get your location.';
        tr.appendChild(td);
        table.appendChild(tr);
    });
} else {
    ...
    const td = document.createElement('td');
    td.setAttribute('colspan', '2');
    td.textContent = 'Geolocation is not supported by this browser.';
    tr.appendChild(td);
    table.appendChild(tr);
}
